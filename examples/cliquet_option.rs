...
fn main() {
    ...
    for cap in [0.01, 0.02, 0.03, 0.05, 0.10] {
        ...
        row(
            &format!("local cap {:.0}%", cap * 100.0),
            swept.analytic_npv().unwrap(),
            None,
        );
    }
    ...
    row(
        "uncapped ratchet (floor 0 only)",
        ratchet.analytic_npv().unwrap(),
        None,
    );
    ...
    row(
        "Heston (same 20% total vol, xi=0.7, rho=-0.7)",
        hp,
        Some(hp_se),
    );
    ...
    row(
        "reverse: 20% coupon + sum of down months",
        rev,
        Some(rev_se),
    );
    ...
    row(
        "napoleon under Heston (same total vol)",
        nap_h,
        Some(nap_h_se),
    );
