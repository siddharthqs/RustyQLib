...
fn main() {
    ...
    common::note(
        "the two models differ slightly by construction — that gap is expected, not a bug",
    );
    ...
    let call = with_divs(base())
        .borrow_cost(0.02)
        .vanilla(PutOrCall::Call)
        .build()
        .expect("option must build");
    let put = with_divs(base())
        .borrow_cost(0.02)
        .vanilla(PutOrCall::Put)
        .build()
        .expect("option must build");
