...
fn is_holiday(date: NaiveDate) -> bool {
    ...
    if date.month() == 12 && date.day() == 25 {
        ...
        // Check if the date is New Year's Day
        return true;
...
impl DayCountConvention {
    ...
    pub fn get_year_fraction(&self, start_date: NaiveDate, maturity_date: NaiveDate) -> f64 {
        ...
        duration.num_days() as f64 / self.num_of_days() as f64
    }
}
...
impl TermStructure {
    ...
    pub fn rates(&self, val_date: NaiveDate) -> Vec<f64> {
        ...
        for i in 0..self.discount_factor.len() {
            let rate = (1.0 / self.discount_factor[i] - 1.0)
                / self.day_count.get_year_fraction(val_date, self.date[i]);
            rates.push(rate);
        }
        rates
    }
    pub fn build_term_structure(
        ...
        TermStructure::new(dates, discount_factor, rate, day_count)
    }
}
...
pub fn convert_mm_to_date(mut date: String) -> NaiveDate {
    ...
    let date_in_months = current_date
        .with_year(new_year)
