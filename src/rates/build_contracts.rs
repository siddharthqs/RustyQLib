...
pub fn build_term_structure(mut contracts: Vec<Box<dyn Rates>>) -> TermStructure {
    let mut ts: rates::utils::TermStructure = rates::utils::TermStructure::new(
        vec![],
        vec![],
        vec![],
    ...
    ts.discount_factor
        .push(contract.get_maturity_discount_factor());
    ts.date.push(contract.get_maturity_date());
    ts.rate.push(contract.get_rate());
    for contract in contracts.iter_mut().skip(1) {
        let contract = contract.as_mut();
