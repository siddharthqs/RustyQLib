...
impl Deposit {
    ...
    pub fn builder(
        ...
        let start_date =
            NaiveDate::parse_from_str(&start_date, "%Y-%m-%d").expect("Invalid date format");
        let maturity_date =
            NaiveDate::parse_from_str(&maturity_date, "%Y-%m-%d").expect("Invalid date format");
    ...
    pub fn get_value(&self) -> f64 {
        ...
        (1.0 + self.fix_rate * self.get_year_fraction(self.start_date)) * self.notional
    }
...
impl Rates for Deposit {
    fn get_implied_rates(&self) -> f64 {
        let curve = self
            .term_structure
            .as_ref()
            .expect("Term structure is not set");
        let df = curve.interpolate_log_linear(self.valuation_date, self.maturity_date);

        (1.0 / df - 1.0) / self.get_year_fraction(self.valuation_date)
    }
