...
pub fn cornish_fisher_var(
    ...
    let z2 = zl * zl;
    let q = zl + (z2 - 1.0) * skewness / 6.0 + zl * (z2 - 3.0) * excess_kurtosis / 24.0
...
pub fn delta_normal_var(
    ...
    assert!(
        variance >= -1e-12,
        "covariance matrix is not PSD on this exposure"
    );
...
mod tests {
    ...
    fn delta_normal_var_and_euler_decomposition() {
        ...
        let variance =
            w[0] * w[0] * cov[0][0] + 2.0 * w[0] * w[1] * cov[0][1] + w[1] * w[1] * cov[1][1];
