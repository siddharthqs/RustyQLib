...
impl<'a> BlackScholesProcess<'a> {
    ...
    pub(crate) fn step_with_vol(
        ...
        let next = match scheme {
            DiscretizationScheme::Exact => s * exp((mu - 0.5 * sigma * sigma) * dt + sigma * dw),
    ...
    fn step_qe(&self, x: &[f64], dt: f64, dw: &[f64], out: &mut [f64]) {
        ...
        let k0 = match branch {
            QeBranch::Quadratic { a, b2 } if 2.0 * a_mc * a < 1.0 => {
                -a_mc * b2 * a / (1.0 - 2.0 * a_mc * a) + 0.5 * (1.0 - 2.0 * a_mc * a).ln()
                    - (k1 + 0.5 * k3) * v
            }
...
impl StochasticProcess for MultiAssetGbmProcess {
    ...
    fn evolve(&self, _t: f64, x: &[f64], dt: f64, dw: &[f64], out: &mut [f64]) {
        for i in 0..self.dim() {
            ...
            out[i] = x[i] * exp((self.drift_rates[i] - 0.5 * sigma * sigma) * dt + sigma * dwi);
        }
    }
}
...
mod tests {
    ...
    fn schemes_match_the_classical_gbm_formulas() {
        ...
        let expect = s * (1.0 + 0.03 * dt + 0.2 * dw + 0.5 * 0.04 * (dw * dw - dt));
    ...
    fn qe_sample(hp: HestonParams, drift: f64, v: f64, dt: f64, n: usize) -> (f64, f64, f64) {
        ...
        (
            mean,
            sum_v2 / n as f64 - mean * mean,
            sum_s / n as f64 / 100.0,
        )
    }
    ...
    fn qe_quadratic_branch_matches_the_cir_moments() {
        ...
        assert!(
            ...
            "test must hit the quadratic branch"
        );
    ...
    fn qe_exponential_branch_matches_the_cir_moments() {
        ...
        assert!(
            s2 / (m * m) > QE_PSI_SWITCH,
            "test must hit the exponential branch"
        );
        let (mean, var, _) = qe_sample(hp, 0.0, v, dt, 200_000);
    ...
    fn qe_spot_step_is_a_martingale_in_both_branches() {
        ...
        assert!(
            (ratio - 1.0).abs() < 5e-3,
            "exponential branch ratio {ratio}"
        );
    }
    ...
    fn multi_gbm_recovers_forwards_and_correlation() {
        ...
        assert!(
            (m0 / nf - t0).abs() / t0 < 1e-3,
            "asset 0 forward {}",
            m0 / nf
        );
        assert!(
            (m1 / nf - t1).abs() / t1 < 1e-3,
            "asset 1 forward {}",
            m1 / nf
        );
        ...
        assert!(
            (corr - rho).abs() < 0.01,
            "log-return correlation {corr} vs {rho}"
        );
    }
