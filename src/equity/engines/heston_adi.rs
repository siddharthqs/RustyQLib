...
// grid assembly runs over `(i, j)` node subscripts (`at(i, j)`,
// `v_grid[j]`); iterator forms would obscure the stencils
...
pub(crate) fn solve(
    ...
    let mixed = move |u: &[f64]| -> Vec<f64> {
        ...
        for i in 1..nx {
            for j in 1..nv {
                let idx = i * sx + j;
                let cross = (u[idx + sx + 1] - u[idx + sx - 1] - u[idx - sx + 1] + u[idx - sx - 1])
                    / (4.0 * dx * dv);
...
mod tests {
    ...
    fn option(
        ...
        engine: Engine,
        american: bool,
    ) -> crate::equity::vanilla_option::EquityOption {
    ...
    fn grid_greeks_match_the_characteristic_function_bumps() {
        ...
        let h = 0.5;
        let bump =
            |ds: f64| heston_price(100.0 + ds, 100.0, 0.05, 0.02, 1.0, &hp(), PutOrCall::Call);
        let delta_ref = (bump(h) - bump(-h)) / (2.0 * h);
        let gamma_ref = (bump(h) - 2.0 * bump(0.0) + bump(-h)) / (h * h);
        assert!(
            (sol.delta - delta_ref).abs() < 5e-3,
            "delta {} vs {delta_ref}",
            sol.delta
        );
        assert!(
            (sol.gamma - gamma_ref).abs() < 5e-4,
            "gamma {} vs {gamma_ref}",
            sol.gamma
        );
    }
    ...
    fn american_put_is_corroborated_by_the_lsmc() {
        ...
        assert!(
            fd > european + 0.05,
            "american fd {fd} vs european {european}"
        );
