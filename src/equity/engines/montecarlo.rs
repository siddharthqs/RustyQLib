...
//!   Andersen QE scheme.
//! - [`stats`] reports the standard error alongside the price.
...
/// Step floor for rough Bergomi. The exact scheme's variance path
/// carries no discretization bias at any resolution (the joint Gaussian
/// is exact on the grid); the floor resolves the spot's
/// integrated-variance sums. Above
/// [`RBergomiGenerator::EXACT_MAX_STEPS`] the engine switches to the
/// FFT-accelerated hybrid scheme, so fine grids stay affordable.
pub const RBERGOMI_MIN_STEPS: usize = 100;
...
/// [`EquityOptionBase::pv_cash_dividends`](crate::equity::vanilla_option::EquityOptionBase::pv_cash_dividends).
...
/// Price under a market view; `None` prices the base market. Bumped
/// views reprice with the same deterministic draws as the base price
/// (common random numbers), so bumped-minus-base differences are free of
/// sampling noise.
pub fn npv(option: &EquityOption, bumped_market: Option<&BumpedMarket>) -> f64 {
    stats(option, bumped_market).pv
}

/// Price with standard error and simulation diagnostics (the batch
/// pricing path reads the standard error from here).
///
...
/// not re-checked here.
pub(crate) fn stats(option: &EquityOption, bumped_market: Option<&BumpedMarket>) -> McStats {
    let base = BumpedMarket::base(&option.market);
    let m = bumped_market.unwrap_or(&base);
    price(option, &market_params(option, m))
}
...
// bumped-market reprices of [`npv`] — every bumped reprice reuses the
// base price's draws.
...
pub(crate) fn pathwise_delta_vega(option: &EquityOption) -> Option<(f64, f64)> {
    ...
    let p = market_params(option, &BumpedMarket::base(&option.market));
    let df = exp(-p.r * p.t);
    ...
    let partials: Vec<(f64, f64)> = z
        ...
        .collect();
    let (delta_sum, vega_sum) = partials
        .into_iter()
...
pub(crate) fn aad_greeks(option: &EquityOption) -> Option<AadGreeks> {
    ...
    match option.model {
        ...
        // local vol's surface interpolation is off the tape; rBergomi's
        // whole-path Cholesky draw has no per-step recursion to tape
        Model::LocalVol | Model::RBergomi(_) => None,
    }
}
                ...
                for (step_idx, dwi) in dw.iter().enumerate() {
                    s = match cfg.scheme {
                        ...
                        DiscretizationScheme::Milstein => {
                            let correction = sigma * sigma * (0.5 * (dwi * dwi - dt));
                            (s * ((r - p.q) * dt + sigma * *dwi + correction + 1.0)).maxf(0.0)
                        }
                    };
        ...
        .map(|chunk| {
            ...
            for i in chunk * PATH_CHUNK..((chunk + 1) * PATH_CHUNK).min(cfg.paths) {
                ...
                for j in 0..steps {
                    ...
                    let dw_v = hp.rho * dw_s + rho_perp * sqrt_dt * sign * z[2 * j + 1];
                    ...
                    v = v + (theta_var - v_pos) * (hp.kappa * dt) + sqrt_v * (hp.vol_of_vol * dw_v);
                    path_vars.push(s);
                }
                let payoff = option
                    .payoff
...
fn bs_process<'a>(option: &'a EquityOption, p: &MarketParams) -> BlackScholesProcess<'a> {
    let vol = match option.model {
        ...
        Model::RBergomi(_) => unreachable!("rBergomi paths are generated by the dedicated routes"),
    };
...
fn effective_steps(cfg: &MonteCarloConfig, model: &Model) -> usize {
    match model {
        ...
        Model::RBergomi(_) => cfg.time_steps.max(RBERGOMI_MIN_STEPS),
...
fn european_npv(option: &EquityOption, p: &MarketParams) -> McStats {
    ...
    if option.model.is_rbergomi() {
        return rbergomi_european_npv(option, p);
    }
...
fn heston_european_npv(option: &EquityOption, p: &MarketParams) -> McStats {
    let hp = option
        .heston_params()
        .with_vol_shift(p.sigma - option.volatility());
    let cfg = option.mc_cfg();
...
/// Rough Bergomi valuation. The vol leg is sampled jointly through a
/// [`RBergomiGenerator`]: the **exact scheme** (one Cholesky factor of
/// the joint Gaussian law per pricing — zero variance-path
/// discretization bias) up to
/// [`RBergomiGenerator::EXACT_MAX_STEPS`] steps, the FFT-accelerated
/// **hybrid scheme** (Bennedsen-Lunde-Pakkanen) on finer grids.
///
/// European vanillas without cash dividends price on the **conditional
/// (Romano-Touzi / mixed) estimator**: conditional on the vol path the
/// terminal spot is lognormal, so each path contributes a Black price
/// with the leverage-adjusted forward and the orthogonal `(1 - rho^2)`
/// share of the integrated variance — the spot's own noise is integrated
/// out analytically, collapsing the variance of the estimator. Every
/// other payoff simulates full spot paths ([`run_rbergomi_paths`]) with
/// the same payoff routing as the Heston route (barrier bridge
/// correction, observation-aligned autocallables/accumulators, generic
/// path payoffs). Vega bumps map to a parallel shift of the forward vol
/// `sqrt(xi0)`.
fn rbergomi_european_npv(option: &EquityOption, p: &MarketParams) -> McStats {
    let rp = option
        .rbergomi_params()
        .with_vol_shift(p.sigma - option.volatility());
    let cfg = option.mc_cfg();
...
/// Bergomi. Per path only the vol leg `(dW^v, VH)` is simulated; the
/// left-point sums
///
/// ```text
/// I = sum sqrt(V_{j-1}) dW^v_j,     Q = sum V_{j-1} dt
/// ```
///
/// give the conditionally lognormal terminal spot: forward
/// `F exp(rho I - rho^2 Q / 2)`, log-variance `(1 - rho^2) Q`, and the
/// path contributes the discounted Black price on those. The
/// discrete-time conditional expectation is exact (the left-point
/// variance is measurable before each increment), so this agrees with
/// the full path simulation at the same grid while integrating out the
/// orthogonal spot noise.
fn rbergomi_vanilla_mixed(
    option: &EquityOption,
    p: &MarketParams,
    rp: &RBergomiParams,
    steps: usize,
    ...
    let cfg = option.mc_cfg();
    let gen = RBergomiGenerator::for_grid(rp.hurst, steps, p.t)
        .expect("validated rBergomi parameters must yield a vol-leg sampler");
    let dt = p.t / steps as f64;
    let df = exp(-p.r * p.t);
    let forward = p.s0 * exp((p.r - p.q) * p.t);
    ...
    let partials: Vec<(f64, f64)> = (0..chunks)
        ...
        .map(|chunk| {
            ...
            for i in chunk * PATH_CHUNK..((chunk + 1) * PATH_CHUNK).min(cfg.paths) {
                ...
                gen.correlate(&z, &mut dwv, &mut volterra, &mut scratch);
                let mut v_left = rp.variance(0.0, 0.0);
...
/// `eval(spots, vols)` receives the path's spot levels and the per-step
/// vols (`sqrt(V)` entering each step, for the barrier bridge). Each
/// path spends `2 * steps` normals on the correlated `(dW^v, VH)` draw
/// and `steps` more on the orthogonal spot noise; the log-Euler spot
/// step uses the left-point variance, so the discounted spot is an exact
/// discrete-time martingale.
fn run_rbergomi_paths<F>(
    option: &EquityOption,
    p: &MarketParams,
    rp: &RBergomiParams,
    steps: usize,
    ...
    F: Fn(&[f64], &[f64]) -> f64 + Sync,
{
    ...
    let gen = RBergomiGenerator::for_grid(rp.hurst, steps, p.t)
        .expect("validated rBergomi parameters must yield a vol-leg sampler");
    let (rho, rho_perp) = (rp.rho, (1.0 - rp.rho * rp.rho).sqrt());
    let sqrt_dt = dt.sqrt();
        ...
        .map(|chunk| {
            ...
            for i in chunk * PATH_CHUNK..((chunk + 1) * PATH_CHUNK).min(cfg.paths) {
                ...
                for j in 0..steps {
                    let vol = v_left.sqrt();
                    vols[j] = vol;
                    let dw = rho * dwv[j] + rho_perp * sqrt_dt * z[2 * steps + j];
                    s *= exp(drift_dt - 0.5 * v_left * dt + vol * dw);
                    if let Some(divs) = &divs {
                        s = (s - divs[j]).max(1e-8);
                    }
                    spots[j] = s;
...
fn american_npv(option: &EquityOption, p: &MarketParams) -> McStats {
    ...
    let steps = if cfg.time_steps > 1 {
        cfg.time_steps
    } else {
        LSMC_DEFAULT_STEPS
    }
    .max(if option.model == Model::LocalVol {
        LOCAL_VOL_MIN_STEPS
    } else {
        1
    });
...
/// share it.
// elimination runs over `m[row][col]` subscripts; iterator forms would
// obscure the math
