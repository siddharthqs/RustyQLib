...
/// Tree build under a market view. A zero-bump view reproduces the base
/// tree bit for bit.
fn setup(option: &EquityOption, m: &BumpedMarket) -> TreeSetup {
    assert!(option.market.spot.mid() >= 0.0);
    let maturity = option.base.maturity_date;
    let t = m.time_to_maturity(maturity).max(1e-6);
    let r = m.risk_free_rate(maturity);
    let b = r - m.carry_yield();
    let s0 = m.effective_spot(maturity);
    ...
    let params = cfg
        ...
        .params(
            ...
            b,
            m.volatility(option.base.strike_price, maturity),
            t,
            n,
        )
...
/// Per-layer early-exercise rule: `(layer, spot, continuation) -> value`.
type ExerciseRule<'a> = Box<dyn Fn(usize, f64, f64) -> f64 + 'a>;
...
fn exercise_rule<'a>(option: &'a EquityOption, t: f64, n: usize) -> Option<ExerciseRule<'a>> {
...
/// applied per step on a variance-equal time grid. The market view's
/// shifts enter the same way as in [`setup`]: parallel on the forward
...
fn term_solution(option: &EquityOption, m: &BumpedMarket) -> LatticeSolution {
    let maturity = option.base.maturity_date;
    let t = m.time_to_maturity(maturity).max(1e-6);
    let s0 = m.effective_spot(maturity);
    let strike = option.base.strike_price;
    let carry = m.carry_yield();
    let curve = &option.market.discount_curve;
    // the parallel rate shift rides on every forward rate; the curve
    // keeps its shape
    let d_rate = m.bump().d_rate;
    let forward_rate = |t1: f64, t2: f64| (curve.df(t1) / curve.df(t2)).ln() / (t2 - t1) + d_rate;
    ...
    let total_variance = |tt: f64| {
        ...
        let sigma = m.vol_at(strike, fwd, tt);
        sigma * sigma * tt
    };
    let lattice = TermLattice::build(
        option.lattice_cfg().steps,
        t,
        &forward_rate,
        &forward_carry,
        &total_variance,
...
/// term-structure lattice when `lattice.term_structure` is set. `None`
/// prices the base market — identical to a zero-bump view, bit for bit.
pub fn npv(option: &EquityOption, bumped_market: Option<&BumpedMarket>) -> f64 {
    ...
    if option.lattice_cfg().term_structure {
        return term_solution(option, m).price;
    }
    let s = setup(option, m);
    let strike = option.base.strike_price;
    let terminal = |spot: f64| option.payoff.payoff(spot, strike);
...
/// Stencil shorthand for the tree Greeks below.
fn npv_bumped(option: &EquityOption, bump: Bump) -> f64 {
    npv(option, Some(&BumpedMarket::new(&option.market, bump)))
}
...
pub fn solution(option: &EquityOption) -> LatticeSolution {
    solution_bumped(option, Bump::NONE)
}

fn solution_bumped(option: &EquityOption, bump: Bump) -> LatticeSolution {
    let m = BumpedMarket::new(&option.market, bump);
    if option.lattice_cfg().term_structure {
        return term_solution(option, &m);
    }
    let s = setup(option, &m);
    let strike = option.base.strike_price;
    let terminal = |spot: f64| option.payoff.payoff(spot, strike);
...
pub fn vega(option: &EquityOption) -> f64 {
    let h = VOL_BUMP;
    (npv_bumped(option, Bump::vol(h)) - npv_bumped(option, Bump::vol(-h))) / (2.0 * h)
}
pub fn rho(option: &EquityOption) -> f64 {
    let h = RATE_BUMP;
    (npv_bumped(option, Bump::rate(h)) - npv_bumped(option, Bump::rate(-h))) / (2.0 * h)
}

/// Vanna from the change in the tree delta under a parallel vol bump.
pub fn vanna(option: &EquityOption) -> f64 {
    let h = VOL_BUMP;
    (solution_bumped(option, Bump::vol(h)).delta - solution_bumped(option, Bump::vol(-h)).delta)
        / (2.0 * h)
}
...
pub fn charm(option: &EquityOption) -> f64 {
    ...
    (solution_bumped(option, Bump::spot(h)).theta - solution_bumped(option, Bump::spot(-h)).theta)
        / (2.0 * h)
}
...
pub fn zomma(option: &EquityOption) -> f64 {
    let h = VOL_BUMP;
    (solution_bumped(option, Bump::vol(h)).gamma - solution_bumped(option, Bump::vol(-h)).gamma)
        / (2.0 * h)
}
...
pub fn volga(option: &EquityOption) -> f64 {
    let h = VOLGA_BUMP;
    (npv_bumped(option, Bump::vol(h)) - 2.0 * npv(option, None) + npv_bumped(option, Bump::vol(-h)))
        / (h * h)
}
...
pub fn npv_with_diagnostics(option: &EquityOption) -> LatticeDiagnostics {
    let s = setup(option, &BumpedMarket::base(&option.market));
    let strike = option.base.strike_price;
    let terminal = |spot: f64| option.payoff.payoff(spot, strike);
    let exercise = exercise_rule(option, s.t, s.n);
...
mod tests {
    ...
    fn european_tree_greeks_match_the_analytic_engine() {
        ...
        let analytic = builder(PutOrCall::Call, Engine::BlackScholes)
            .build()
            .unwrap();
        ...
        assert_eq!(
            result.pv,
            tree.npv(),
            "pricing_result must reuse the npv price"
        );
    ...
    fn american_put_greeks_reflect_the_exercise_boundary() {
        let american = builder(PutOrCall::Put, Engine::Binomial)
            .american()
            .build()
            .unwrap();
    ...
    #[test]
    fn zero_bump_reprice_reproduces_npv() {
        use crate::equity::bump::{Bump, BumpedMarket};
        let option = builder(PutOrCall::Put, Engine::Binomial)
            .american()
            .build()
            .unwrap();
        let zero = option.price_bumped(&BumpedMarket::base(&option.market));
        assert_eq!(zero, option.npv());
        // a spot shift moves the reprice in the direction of delta
        let bumped = option.price_bumped(&BumpedMarket::new(&option.market, Bump::spot(1.0)));
