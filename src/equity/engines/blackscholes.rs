...
mod tests {
    ...
    fn binomial_matches_analytic_vanilla_and_binary() {
        for pc in [PutOrCall::Call, PutOrCall::Put] {
            ...
            assert!(
                (tree - analytic).abs() < 0.02,
                "vanilla {pc:?}: tree={tree} bs={analytic}"
            );
            ...
            assert!(
                (tree - analytic).abs() < 0.02,
                "binary {pc:?}: tree={tree} bs={analytic}"
            );
        }
    }
    ...
    fn monte_carlo_sobol_beats_default_tolerance_and_is_reproducible() {
        ...
        assert_eq!(
            first, second,
            "deterministic sampler must reproduce exactly"
        );
    ...
    fn monte_carlo_greeks_match_analytic() {
        ...
        assert!(
            (option.delta() - 0.6242517279).abs() < 0.01,
            "delta {}",
            option.delta()
        );
        assert!(
            (option.gamma() - 0.0126477644).abs() < 0.003,
            "gamma {}",
            option.gamma()
        );
        assert!(
            (option.vega() - 37.9432933117).abs() < 1.0,
            "vega {}",
            option.vega()
        );
        assert!(
            (option.theta() - -8.1011898970).abs() < 0.5,
            "theta {}",
            option.theta()
        );
        assert!(
            (option.rho() - 48.1939180046).abs() < 0.5,
            "rho {}",
            option.rho()
        );
    }
    ...
    fn lsmc_american_put_close_to_tree_and_dominates_european() {
        ...
        assert!(
            lsmc > european,
            "lsmc {lsmc} must exceed european {european}"
        );
    ...
    fn quoted_option(k: f64, maturity: NaiveDate, market_price: f64) -> Box<EquityOption> {
        let mut option = test_option(PutOrCall::Call, flat_5pct());
    ...
    fn barrier_option(
        ...
        option.market.dividend_yield = 0.02; // match the oracle setup
        option
    }
    ...
    fn double_barriers_and_rebates_price_across_engines() {
        ...
        let dko = build()
            .double_barrier(PutOrCall::Call, KnockType::Out, 85.0, 120.0)
        ...
        let mc_px = mc.npv();
        assert!(
            mc_px > dko.npv() - 0.02 && (mc_px - dko.npv()).abs() < 0.30,
            "mc {mc_px} vs analytic {}",
            dko.npv()
        );
        ...
        assert!(
            (mc_rebate.npv() - rebated.npv()).abs() < 0.30,
            "mc {} vs analytic {}",
            mc_rebate.npv(),
            rebated.npv()
        );
        ...
        let at_hit = build()
            ...
            .expect("option must build");
        assert!(
            at_hit.npv() > rebated.npv(),
            "earlier payment is worth more"
        );
    }
    ...
    fn lookback_analytic_engine_matches_the_closed_forms_and_mc_converges() {
        ...
        // is enormous
        assert!(
            float_call.delta() > 0.1 && float_call.delta() < 0.4,
            "{}",
            float_call.delta()
        );
        ...
        assert!(
            coarse_px < direct && fine_px < direct,
            "{coarse_px} {fine_px} vs {direct}"
        );
        assert!(
            direct - fine_px < direct - coarse_px,
            "finer grid must close the gap"
        );
        ...
        assert!(
            mc_px < direct_put && direct_put - mc_px < 1.5,
            "{mc_px} vs {direct_put}"
        );
    }
    ...
    fn arithmetic_average_dominates_geometric_on_same_paths() {
        ...
        assert!(
            price_mc(Arithmetic) > price_mc(Geometric),
            "AM-GM inequality"
        );
    }
    ...
    fn fd_grid_greeks_match_analytic_for_european() {
        ...
        assert!(
            (option.delta() - 0.6242517279).abs() < 1e-3,
            "delta {}",
            option.delta()
        );
        assert!(
            (option.gamma() - 0.0126477644).abs() < 1e-4,
            "gamma {}",
            option.gamma()
        );
        assert!(
            (option.theta() - -8.1011898970).abs() < 0.03,
            "theta {}",
            option.theta()
        );
        assert!(
            (option.vega() - 37.9432933117).abs() < 0.05,
            "vega {}",
            option.vega()
        );
        assert!(
            (option.rho() - 48.1939180046).abs() < 0.05,
            "rho {}",
            option.rho()
        );
    }
    ...
    fn fd_grid_is_configurable() {
        ...
        assert!(
            (coarse.npv() - 14.2312547860).abs() < 0.02,
            "{}",
            coarse.npv()
        );
    }
    ...
    fn parallel_paths_are_bit_reproducible() {
        for sampler in [
            ...
            crate::equity::montecarlo::Sampler::PseudoRandom,
        ] {
            let mut option = test_option(PutOrCall::Call, flat_5pct());
            option.engine = crate::equity::utils::PricingEngine::from_kind(Engine::MonteCarlo);
    ...
    fn heston_barrier_and_asian_price_on_mc() {
        ...
        let ko_price = ko.npv();
        assert!(
            ko_price > 0.0 && ko_price < vanilla,
            "ko={ko_price} vanilla={vanilla}"
        );
    ...
    fn heston_american_put_degenerates_to_the_lattice_price() {
        ...
        assert!(
            (lsmc - reference).abs() < 0.2,
            "lsmc={lsmc} lattice={reference}"
        );
    }
    ...
    fn dividend_paying_option(pc: PutOrCall) -> EquityOption {
        let mut option = test_option(pc, flat_5pct());
        option.market.cash_dividends = vec![(NaiveDate::from_ymd_opt(2026, 7, 1).unwrap(), 3.0)];
        option
    }
    ...
    fn futures_option_rejects_non_analytic_engine() {
        let mut option = futures_option(
            PutOrCall::Call,
            crate::equity::black76::FuturesSettlement::Discounted,
        );
        option.engine = crate::equity::utils::PricingEngine::from_kind(Engine::MonteCarlo);
        option.npv();
    }
    ...
    fn forward_start_heston_degenerates_to_black_scholes() {
        ...
        heston.engine = crate::equity::utils::PricingEngine::MonteCarlo(
            crate::equity::montecarlo::MonteCarloConfig {
                paths: 50_000,
                ..Default::default()
            },
        );
    ...
    fn autocall_that_always_calls_pays_coupon_at_first_observation() {
        ...
        let stats = crate::equity::montecarlo::stats(&note, None);
    ...
    fn autocall_value_increases_with_coupon_and_lower_protection() {
        ...
        assert!(
            autocall_note(105.0, 70.0, 8.0).npv() > base,
            "higher coupon"
        );
        assert!(
            autocall_note(105.0, 50.0, 5.0).npv() > base,
            "lower knock-in barrier"
        );
    }
    ...
    fn heston_adjoint_greeks_match_the_characteristic_function() {
        ...
        let delta_cf =
            crate::equity::heston::native_vanilla_delta(&heston_vanilla(PutOrCall::Call)).unwrap();
        assert!(
            (g.delta - delta_cf).abs() < 0.01,
            "delta {} vs {delta_cf}",
            g.delta
        );
    ...
    fn american_put_fd_and_tree_agree_and_dominate_european() {
        ...
        assert!(
            fd > european_put,
            "american {fd} must exceed european {european_put}"
        );
