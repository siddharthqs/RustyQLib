...
mod tests {
    ...
    fn volga_matches_vega_bump_and_smile_sign() {
        for s in [FuturesSettlement::Discounted, FuturesSettlement::Margined] {
            ...
            let h = 1e-5;
            let bump = (vega(F, K, R, SIG + h, T, s) - vega(F, K, R, SIG - h, T, s)) / (2.0 * h);
        ...
        // golden values, bump-verified against an independent reference
        assert!((volga(F, K, R, SIG, T, FuturesSettlement::Discounted) + 2.81430260).abs() < 1e-6);
        assert!((volga(F, K, R, SIG, T, FuturesSettlement::Margined) + 2.95859498).abs() < 1e-6);
    }
    ...
    fn settlement_parses_from_strings() {
        assert_eq!(
            "discounted".parse::<FuturesSettlement>().unwrap(),
            FuturesSettlement::Discounted
        );
        assert_eq!(
            "black76".parse::<FuturesSettlement>().unwrap(),
            FuturesSettlement::Discounted
        );
        assert_eq!(
            "margined".parse::<FuturesSettlement>().unwrap(),
            FuturesSettlement::Margined
        );
        assert_eq!(
            "futures_style".parse::<FuturesSettlement>().unwrap(),
            FuturesSettlement::Margined
        );
