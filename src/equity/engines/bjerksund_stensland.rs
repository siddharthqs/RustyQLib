...
pub fn price(s: f64, k: f64, r: f64, q: f64, sigma: f64, t: f64, put_or_call: PutOrCall) -> f64 {
    ...
    value
        .max(intrinsic)
        .max(bs_price(s, k, r, q, sigma, t, put_or_call))
}
...
/// principle.
#[allow(clippy::too_many_arguments)] // the paper's phi(S, T, gamma, H, I | r, b, sigma)
...
fn psi(
    ...
    (lambda * t2).exp()
        * s.powf(gamma)
        * (bivariate_norm_cdf(-d1, -f1, rho)
            - (i2 / s).powf(kappa) * bivariate_norm_cdf(-d2, -f2, rho)
...
mod tests {
    ...
    fn is_a_lower_bound_that_tracks_the_tree() {
        ...
        for (pc, s, k, r, q, v, t) in cases {
            ...
            assert!(
                approx <= tree + 5e-3,
                "{pc:?} s={s}: approx {approx} above tree {tree}"
            );
            assert!(
                tree - approx < 0.10,
                "{pc:?} s={s}: approx {approx} vs tree {tree}"
            );
        }
    }
    ...
    fn engine_dispatch_prices_and_greeks() {
        ...
        assert!(
            (bs2002.delta() - fd.delta()).abs() < 0.01,
            "delta {} vs fd {}",
            bs2002.delta(),
            fd.delta()
        );
