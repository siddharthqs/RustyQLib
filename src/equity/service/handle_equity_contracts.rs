...
use crate::core::utils::{CombinedContract, Contract, ContractOutput};
...
pub fn handle_equity_contract(data: &Contract) -> serde_json::Value {
    let priced =
        std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| price_equity_contract(data)));
...
mod tests {
    ...
    fn unsupported_engine_combination_reports_error() {
        ...
        let err = out["output"]["error"].as_str().expect("error must be set");
        assert!(
            err.contains("MonteCarlo"),
            "should point at the right engine: {err}"
        );
    }
    ...
    fn explicit_valuation_date_prices_reproducibly() {
        ...
        assert!(
            (pv - 14.2312547860).abs() < 1e-8,
            "pv {pv} must be date-independent"
        );
    }
    ...
    fn tree_type_flows_through_the_contract() {
        ...
        let crr = priced("CRR");
        let (lr_pv, crr_pv) = (
            lr["output"]["pv"].as_f64().unwrap(),
            crr["output"]["pv"].as_f64().unwrap(),
        );
        assert!(
            (lr_pv - crr_pv).abs() < 0.05,
            "schemes agree loosely: {lr_pv} vs {crr_pv}"
        );
