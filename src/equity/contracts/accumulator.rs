            ...
            value += match self.side {
                AccumulatorSide::Accumulator => {
                    ...
                    let uop = barrier_price(
                        ...
                        BarrierDirection::Up,
                        KnockType::Out,
                        crate::core::trade::PutOrCall::Put,
                    );
...
mod tests {
    ...
    fn analytic_strip_tracks_dense_monte_carlo_with_the_barrier() {
        ...
        assert!(
            mc > analytic - 3.0 * se,
            "discrete KO should not lose value"
        );
        assert!(
    ...
    fn risk_features_move_the_price_the_right_way() {
        ...
        tight.barrier = 103.0;
        assert!(
            tight.analytic_npv() > baseline,
            "tight KO should truncate the toxic tail"
        );
    ...
    fn payoff_accrues_daily_and_stops_without_accruing_at_knockout() {
        ...
        assert!(
            (value - (5.0 * 0.99 - 14.0 * 0.98)).abs() < 1e-12,
            "{value}"
        );
    ...
    fn equity_option_route_tracks_the_standalone_reference() {
        ...
        assert!(
            mc < reference + 1.0,
            "discrete KO {mc} vs continuous {reference}"
        );
    }
    ...
    fn accumulator_reprices_in_the_market_context_and_stresses_sensibly() {
        ...
        assert!(
            results[0].stress_pnl < 0.0,
            "crash pnl {:?}",
            results[0].stress_pnl
        );
        // the geared holder is short vol (short the wings)
        assert!(
            results[1].stress_pnl < 0.0,
            "vol pnl {:?}",
            results[1].stress_pnl
        );
        // labels identify the product in reports
        assert!(
            results[0].trades[0].label.contains("Accumulator"),
            "{}",
            results[0].trades[0].label
        );
    }
