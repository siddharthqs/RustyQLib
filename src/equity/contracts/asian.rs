...
pub fn geometric_average_strike_price(
    ...
    let e_average = s
        * ((b - 0.5 * sigma * sigma) * t * mean_factor + 0.5 * sigma * sigma * t * var_factor)
            .exp();
...
mod tests {
    ...
    fn arithmetic_average_strike_orders_against_geometric_by_am_gm() {
        ...
        assert!(
            arith_call < geo_call,
            "call: arith {arith_call} vs geo {geo_call}"
        );
        ...
        assert!(
            arith_put > geo_put,
            "put: arith {arith_put} vs geo {geo_put}"
        );
    }
    ...
    fn geometric_golden_values() {
        ...
        assert!(
            (geometric_asian_price(S, 100.0, R, Q, SIG, T, Some(252), PutOrCall::Call) - 6.976295)
                .abs()
                < 1e-5
        );
        assert!(
            (geometric_asian_price(S, 100.0, R, Q, SIG, T, None, PutOrCall::Call) - 6.953600).abs()
                < 1e-5
        );
    }
