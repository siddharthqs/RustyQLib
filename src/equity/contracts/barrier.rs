...
pub fn double_barrier_price(
    ...
    for n in -5i32..=5 {
        ...
        let d3 =
            ((l.powi(2 * n + 2) / (k * s * un * un)).ln() + (b + 0.5 * sigma * sigma) * t) / st;
        let d4 =
            ((l.powi(2 * n + 2) / (f * s * un * un)).ln() + (b + 0.5 * sigma * sigma) * t) / st;
        if call {
            spot_sum +=
                ratio1 * (norm_cdf(d1) - norm_cdf(d2)) - ratio2 * (norm_cdf(d3) - norm_cdf(d4));
            strike_sum += (un / ln).powf(mu - 2.0) * (norm_cdf(d1 - st) - norm_cdf(d2 - st))
                - (l.powi(n + 1) / (un * s)).powf(mu - 2.0)
                    * (norm_cdf(d3 - st) - norm_cdf(d4 - st));
        } else {
            // put: the payoff region is [l, k], integrated with positive
            // normal arguments (d2 anchors the floor f = l, d1 the strike)
            spot_sum +=
                ratio1 * (norm_cdf(d2) - norm_cdf(d1)) - ratio2 * (norm_cdf(d4) - norm_cdf(d3));
            strike_sum += (un / ln).powf(mu - 2.0) * (norm_cdf(d2 - st) - norm_cdf(d1 - st))
                - (l.powi(n + 1) / (un * s)).powf(mu - 2.0)
                    * (norm_cdf(d4 - st) - norm_cdf(d3 - st));
        }
    }
...
mod tests {
    ...
    fn rebate_identities_hold_exactly() {
        ...
        for (dir, h) in [(Down, 85.0), (Up, 115.0)] {
            ...
            let ko_exp = barrier_rebate_value(
                S,
                h,
                rebate,
                R,
            ...
            assert!(
                (ki + ko_exp - rebate * (-R * T).exp()).abs() < 1e-12,
                "{dir:?}"
            );
            // paying at the touch is worth more than waiting until expiry
            let ko_hit = barrier_rebate_value(
                S,
                h,
                rebate,
                R,
    ...
    fn rebate_at_hit_matches_a_first_touch_monte_carlo() {
        ...
        let analytic = barrier_rebate_value(
            S,
            h,
            rebate,
            R,
        ...
        assert!(
            mc < analytic + 0.02,
            "discrete monitoring should undercount"
        );
        assert!(
            (mc - analytic).abs() < 0.20,
    ...
    fn double_barrier_limits_reproduce_single_barriers() {
        for pc in [PutOrCall::Call, PutOrCall::Put] {
            for k in [90.0, 100.0, 110.0] {
                ...
                let dko = double_barrier_price(S, k, 10.0, 120.0, R, Q, SIG, T, KnockType::Out, pc);
                ...
                let dko2 =
                    double_barrier_price(S, k, 80.0, 1000.0, R, Q, SIG, T, KnockType::Out, pc);
                let down_out = barrier_price(
                    S,
                    k,
                    80.0,
                    R,
                ...
                assert!(
                    (dko2 - down_out).abs() < 1e-6,
                    "{pc:?} K={k}: {dko2} vs DO {down_out}"
                );
                // both far: the vanilla
                let wide =
                    double_barrier_price(S, k, 10.0, 1000.0, R, Q, SIG, T, KnockType::Out, pc);
    ...
    fn double_barrier_parity_and_bounds() {
        for pc in [PutOrCall::Call, PutOrCall::Put] {
            ...
            assert!(
                out <= uo + 1e-10 && out <= down_out + 1e-10,
                "{pc:?} bounds"
            );
            assert!(out > 0.0);
        }
    ...
    fn double_barrier_matches_a_dense_monte_carlo() {
        ...
        for pc in [PutOrCall::Call, PutOrCall::Put] {
            ...
            assert!(
                (mc - analytic).abs() < 0.25,
                "{pc:?}: mc {mc} vs analytic {analytic}"
            );
        }
    }
