...
impl WorstOfAutocallable {
    ...
    fn mc_stats_with(&self, p: &Params) -> McStats {
        ...
        let steps =
            self.mc.time_steps.max(PATH_DEPENDENT_MIN_STEPS).div_ceil(n_obs) * n_obs;
        let dt = t / steps as f64;
        let (obs_idx, dfs) = self.observation_grid(t, p.dr, steps);
        let r = self.discount_curve.zero_rate_with(t, Compounding::Continuous) + p.dr;
        ...
        McStats { pv: mean, std_err: (var / nf).sqrt(), paths: self.mc.paths, steps }
    }
...
mod tests {
    ...
    fn note(n: usize, rho: f64, paths: usize) -> WorstOfAutocallable {
        ...
        WorstOfAutocallable::new(
            ...
            mat,
            val,
            YieldCurve::flat(0.03, val, DayCountConvention::Act365, Compounding::Continuous)
                .unwrap(),
            MonteCarloConfig {
                paths,
